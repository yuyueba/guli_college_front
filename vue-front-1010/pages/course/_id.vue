      ...
      <div class="mt20 c-infor-box">
                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="course-txt-body" v-for="comment in comments.items" :key="comment.id">
                            {{comment.nickname}}:       <a href="javascript: void(0)" :title="comment.content">
        ...
        <!-- undisable这个class是否存在，取决于数据属性hasPrevious -->
                <a
                  ...
                  href="#"
                  title="首页"
                  @click.prevent="getPageComments(1)">首</a>
                 <a
                  :class="{undisable: !comments.hasPrevious}"
                  href="#"
                  title="前一页"
                  @click.prevent="getPageComments(comments.current-1)">&lt;</a>
                <a
                  ...
                  :title="'第'+page+'页'"
                  href="#"
                  @click.prevent="getPageComments(page)">{{ page }}</a> 
                <a
                ...
                <a
                  ...
                  href="#"
                  title="末页"
                  @click.prevent="getPageComments(comments.pages)">末</a>
                <div class="clear"/>
              </div>
  ...
  created() {
   ...
   methods:{
      getPageComments(page = 1) {
        this.page = page        
        commentApi.getCommentList(this.page, this.limit,this.courseId)
                    .then(response=>{
      ...
      submit() {
        ...
        this.eduComment.teacherId = this.courseWebVo.teacherId
        this.eduComment.content = this.content
        
        
        commentApi.publicComment(this.eduComment)
                  .then(response=>{
                    this.$message({
                        type: 'success',
                        message: '添加成功!'
